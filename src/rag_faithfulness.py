...
def evaluate_faithfulness(query: str, retrieved_context: str, generated_answer: str) -> str:
    ...
    Detects hallucination risk by checking if answer claims are
    grounded in the retrieved context.

    NOT a replacement for a trained NLI model — this is a heuristic
    pre-filter to surface obvious mismatches before LLM evaluation.
    """
    ...
    for claim in claims:
        # Extract significant content words (length > 4, not stopwords)
        words = [w.lower() for w in re.findall(r'\b\w+\b', claim) if len(w) > 4]
        if not words:
            continue
        # A claim is "grounded" if at least half its key terms appear in context
        ...
        if grounding_ratio < 0.4:
            ungrounded.append(f"  ❌ Possibly hallucinated: '{claim[:80]}...' (only {int(grounding_ratio*100)}% terms in context)")
        elif grounding_ratio < 0.7:
            feedback.append(f"  ⚠️  Weakly grounded: '{claim[:80]}...' ({int(grounding_ratio*100)}% terms in context)")

    # Check if query topic is addressed
    ...
    if not ungrounded and not feedback:
        ...
        lines.append("\nRECOMMENDATION: Re-retrieve with broader query or ask the LLM to cite specific context passages.")
